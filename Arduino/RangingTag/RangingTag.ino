...
void loop() {
    ...
    if (receivedAck) {
        ...
        if (msgId == POLL_ACK) {
            digitalWrite(LED_BUILTIN, HIGH);  digitalWrite(LED_PIN, HIGH);// DEBUG LED ON
            DW1000.getReceiveTimestamp(timePollAckReceived);
            expectedMsgId = RANGE_REPORT;
            transmitRange();
            noteActivity();
            digitalWrite(LED_BUILTIN, LOW); digitalWrite(LED_PIN, LOW); // DEBUG LED OFF
            delay(20); // --- EXTRA DELAY TO COPE WITH UART DIFFICULTIES
